...
The `home-routes` handler in the `prime-factorization.routes.home` namespace
...
The `home-page` function will in turn call the `prime-factorization.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `prime-factorization.handler` namespace:
...
Request middleware functions are located under the `prime-factorization.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `prime-factorization.dev-middleware` namespace found in
